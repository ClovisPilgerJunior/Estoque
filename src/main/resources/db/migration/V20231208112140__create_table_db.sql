    ...
    create table item_ordem_aviamento_aud (
        revtype smallint,
    ...
    create table item_ordem_aviamento (
        setor smallint not null check (setor between 0 and 4),
        data_saida timestamp(6),
        id bigserial not null,
        ordem_aviamento_id bigint not null,
    ...
    create table ordem_aviamento_aud (
        preco_unitario_ordem_aviamento float(53),
        revtype smallint,
        status_ordem_aviamento smallint check (status_ordem_aviamento between 0 and 4),
        data_ordem_aviamento timestamp(6),
        id bigint not null,
        numeroop bigint,
        quantidade_ordem_aviamento bigint,
        rev bigint not null,
        desc_ordem_aviamento varchar(255),
        tecido_ordem_aviamento varchar(255),
        primary key (id, rev)
    );
    ...
    alter table if exists combinacao_detalhe
       add constraint FK35kn7x1qnqmomnyaoqyxgs7kd
       foreign key (combinacao_id)
       references combinacao;
    ...
    alter table if exists item_ordem_aviamento_aud
       add constraint FKlg2nokkeq8ylvkdwvm087mpt9
       foreign key (rev)
       references revisao;
    ...
    alter table if exists item_ordem_aviamento
       add constraint FKdlumbw18hfjmc4gofq4rgmcuv
    ...
    alter table if exists ordem_aviamento_aud
       add constraint FKarx08lgurfccfangbiver9fg0
       foreign key (rev)
       references revisao;
    ...
    alter table if exists produto_saida
       add constraint FK1bwsv4vf2n8nng6vgxniujpr1
       foreign key (ordem_aviamento_id)
       references ordem_aviamento;
